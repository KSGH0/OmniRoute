...
- **What:** `provider_model_speed` table stores per-`model_id`/`provider_id` `p95_ms`/`error_rate`; `POST /api/combos/:id/rank` + scheduler (`startRankedAutobalanceScheduler`, interval 15m + 30s debounce on `triggerRerankForModel`) reorders `combo_targets.sort_order` for combos with `config.rankedAutobalance.autoRank=true` (source `comboName` or `sourceModelId`), `strategy: priority|weighted`. Free vs Paid split via two combos (free $0 vs paid >0 via `pricing` table). Model picker now fetches `GET /api/pricing` and shows `Free • $0.00`(sky) vs`$in/$out`(amber) badges display-only, gated by`settings.showPricingBadges` (default ON) under **Configuration → AI Settings** (`settings/ai/page.tsx`+`RankedPricingAiSettingsTab.tsx`). Global ranked kill-switch `settings.rankedAutobalanceEnabled`also lives under **Configuration → AI Settings**; per-combo toggle remains in`Combos → Control Center` (`RankedAutobalanceToggle.tsx`).
